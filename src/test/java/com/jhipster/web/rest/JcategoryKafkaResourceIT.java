...
import com.jhipster.config.KafkaProperties;
...
class JcategoryKafkaResourceIT {
    ...
    void setup() {
        ...
        JcategoryKafkaResource kafkaResource = new JcategoryKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jcategory-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jcategory-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
